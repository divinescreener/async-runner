...
async def test_run_process_cancellation():
    ...
    with patch('async_runner.core.anyio.get_cancelled_exc_class', return_value=MockCancelledException):
        cancel_exc = MockCancelledException("Task cancelled")
        
        # Setup mock to raise the cancellation exception
        with patch('anyio.open_process', side_effect=cancel_exc):
            with pytest.raises(MockCancelledException):
...
async def test_empty_stdout_lines():
    ...
    with patch('anyio.open_process', return_value=mock_context):
        ...
        empty_stdout_logs = [call for call in mock_logger.info_calls if "STDOUT: " in call and call.endswith("STDOUT: ")]
        empty_stderr_logs = [call for call in mock_logger.error_calls if "STDERR: " in call and call.endswith("STDERR: ")]
