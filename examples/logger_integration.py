...
async def standard_logging_example():
    ...
    # Run some commands
    await run_process(
        ["python", "-c", "print('Standard logging output')"],
        capture_output=True,
        process_name="Standard Logger Test"
    )
...
async def silent_logging_example():
    ...
    success = await run_process(
        ["echo", "This output will be suppressed"],
        capture_output=True,
        process_name="Silent Test"
    )
...
async def logger_switching_example():
    ...
    for logger_name, logger in loggers:
        ...
        await run_process(
            ["echo", f"Message from {logger_name} logger"],
            capture_output=True,
            process_name=f"{logger_name} Test"
        )
