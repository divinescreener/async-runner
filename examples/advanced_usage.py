...
async def new_session_example():
    ...
    success = await run_process(
        ["python", "-c", "import time; print('Background process started'); time.sleep(0.5); print('Background process done')"],
...
async def error_recovery_example():
    ...
    async def retry_command(command, max_retries=3):
        for attempt in range(max_retries):
            ...
            success = await run_process(
                command,
                capture_output=True,
                process_name=f"Retry Attempt {attempt + 1}"
            )
            if success:
    ...
    await retry_command([
        "python", "-c", 
        "import random; exit(0 if random.random() > 0.7 else 1)"
    ])
